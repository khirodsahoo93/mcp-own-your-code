...
Also:
  register_project — index a codebase (AST scan of .py/.ts/.js/.go files)
...
mcp = FastMCP("own-your-code")


# ── Pydantic models for nested inputs ─────────────────────────────────────────
...
def register_project(
    ...
    return _register_project({
        "path": path, "name": name, "languages": languages,
        "include_globs": include_globs, "ignore_dirs": ignore_dirs,
    })
...
def record_intent(
    ...
    confidence: Annotated[int, Field(ge=1, le=5)] = 5,
) -> str:
...
def mark_file_reviewed(
    ...
    return _mark_file_reviewed({"project_path": project_path, "file": file, "note": note})
...
def record_evolution(
    ...
    return _record_evolution({
        "project_path": project_path, "file": file, "function_name": function_name,
        "change_summary": change_summary, "reason": reason, "triggered_by": triggered_by,
    })
...
def explain_function(
    ...
    return _explain_function({"project_path": project_path, "function_name": function_name, "file": file})
...
def get_codebase_map(project_path: str) -> str:
    return _get_codebase_map({"project_path": project_path})
...
def find_by_intent(
    ...
    semantic_weight: Annotated[float, Field(ge=0.0, le=1.0)] = 0.5,
) -> str:
    return _find_by_intent({
        "project_path": project_path, "query": query, "mode": mode,
...
def embed_preflight(
    ...
    return _embed_preflight({"project_path": project_path, "model": model})
...
def embed_intents(
    ...
    return _embed_intents({"project_path": project_path, "model": model})
...
def get_evolution(
    ...
    return _get_evolution({"project_path": project_path, "function_name": function_name, "file": file})
...
# ── implementations ────────────────────────────────────────────────────────────
# Handler logic unchanged — FastMCP validated inputs before reaching here.
...
def _mark_file_reviewed(args: dict) -> str:
    ...
    return json.dumps({"status": "cleared", "file": file, "note": args.get("note")}, indent=2)
...
def _get_codebase_map(args: dict) -> str:
    ...
    return json.dumps({
        ...
        "features": [
            {"title": f["title"], "description": f["description"], "functions": f["functions"]}
...
def _embed_preflight(args: dict) -> str:
    ...
    if deps_ok and pending > 500:
        warnings.append(f"Very large backlog ({pending} intents): expect long runtime and high RAM on CPU-only machines.")
    elif deps_ok and pending > 200:
        warnings.append(f"Large backlog ({pending} intents): embedding may take several minutes and use significant RAM.")
    elif deps_ok and pending > 50:
        warnings.append(f"{pending} intents to embed — may take a few minutes on a laptop CPU.")
    if not deps_ok:
...
def _annotate_existing(args: dict) -> str:
    ...
    return json.dumps({"status": "annotated", "file": file, "recorded": recorded, "errors": errors}, indent=2)
